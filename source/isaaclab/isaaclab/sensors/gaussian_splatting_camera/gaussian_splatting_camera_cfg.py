...
from .gaussian_splatting_camera import GaussianSplattingCamera


@configclass
class GaussianSplattingCameraCfg(SensorBaseCfg):
    ...
    class_type: type = GaussianCamera
    ...
    * ``"typeA : * ; * : labelA"``: All prims with semantic type "typeA" or with label "labelA"
    ...
    """
    Path to the point cloud file. If not None, the point cloud data will be loaded from the file.
    """
    
    gaussian_rotation: tuple[float, float, float, float] = None
    """
    Rotation of the gaussian camera in quaternion (w, x, y, z) format.
    """
