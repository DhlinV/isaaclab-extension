...
from .gaussian_splatting_camera_data import GaussianSplattingCameraData
...
def quaternion_to_rotation_matrix(q):
    ...
    Returns:
        R: Tensor of shape (3,3), representing the rotation matrix.
    """
    w, x, y, z = q
    R = torch.tensor([
...
def rotate_points(points, q):
    """
    Rotates a set of 3D points using a quaternion.
    
    Args:
        points: Tensor of shape (N, 3), representing the point cloud.
        q: Tensor of shape (4,), representing the quaternion (w, x, y, z).
    
    Returns:
        rotated_points: Tensor of shape (N, 3), representing the rotated point cloud.
    """
    R = quaternion_to_rotation_matrix(q).to(points.device) # Convert quaternion to rotation matrix
    return torch.matmul(points.float(), R.float().T)  # Apply rotation



class GaussianSplattingCamera(SensorBase):
    ...
    def __init__(self, cfg: GaussianCameraCfg):
        ...
        self._data = GaussianSplattingCameraData()
        
        import os
        self.position = torch.load(os.path.join(self.cfg.point_cloud_path,  "position.pt")).float()
        self.scale = torch.load(os.path.join(self.cfg.point_cloud_path, "scale.pt")).float()
        self.color = torch.load(os.path.join(self.cfg.point_cloud_path, "color.pt")).float()
        self.rot = torch.load(os.path.join(self.cfg.point_cloud_path, "rot.pt")).float()
        self.opacity = torch.load(os.path.join(self.cfg.point_cloud_path, "opacity.pt")).float()

        print("[INFO] [Pytorch] Point cloud loaded")
    ...
    @property
    def data(self) -> GaussianSplattingCameraData:
    ...
    def quaternion_multiply(q1, q2):
        """
        Compute quaternion multiplication q' = q2 ⊗ q1 (applying q2 to q1).
        q1: (N, 4) quaternion array
        q2: (4,) single quaternion
        Returns: (N, 4) rotated quaternions
        """
        w2, x2, y2, z2 = q2  # `q2` is now applied first
        ...
        return torch.vstack((w, x, y, z)).T  # Shape: (N, 4)
    ...
    def _initialize_impl(self):
        ...
        rot = np.array(self.cfg.gaussian_rotation)
        self.gaussian_rotation = torch.from_numpy(rot).to(self._device).float()
        # self.position = rotate_points(self.position, rot.astype(np.float32))
        self.scale = self.scale.to(self._device)
        self.color = self.color.to(self._device)
        self.rot = self.rot.to(self._device)
        # self.rot = self.quaternion_multiply(self.rot, torch.from_numpy(rot).to(self._device))
        self.opacity = self.opacity.to(self._device)
    ...
    def _check_supported_data_types(self, cfg: GaussianCameraCfg):
        """Checks if the data types are supported by the ray-caster camera."""
        # check if there is any intersection in unsupported types
        # reason: these use np structured data types which we can't yet convert to torch tensor
        common_elements = set(cfg.data_types) & GaussianCamera.UNSUPPORTED_TYPES
        if common_elements:
    ...
    def _process_annotator_output(self, name: str, output: Any) -> tuple[torch.tensor, dict | None]:
        ...
        if name == 'camera_params':
            data = output['cameraViewTransform']
            info = None
